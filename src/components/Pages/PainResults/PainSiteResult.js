...
function PainSiteResult(props) {
  const { painData } = props;
  ...
  async function drawImage(canvas, imageData) {
    ...
    const backgroundImage = new CanvasImage(canvas, painData.canvasInfo.height, painData.canvasInfo.width);
  ...
  async function handleCanvas(canvas) {
    if (canvas) {
      ...
      canvas.height = Math.floor(preferredWidth * painData.canvasInfo.ratio);
      ...
      await drawImage(canvas, painData.imageData);
    }
  }
  const LeftContent = () => <Avatar.Text size={42} label={painData.id} />;
  ...
  return (
    <View>
      <Card>
        <Card.Title title="Pain Site" left={LeftContent} />
        <Card.Content
        ...
        >
          <View style={styles.row}>
            <Surface style={styles.infoBox}>
              <Title>Pain Severity: {Math.round(painData.severity)}%</Title>
              <ProgressBar progress={painData.severity / 100} color={painData.painColour} style={{ marginTop: 10 }} />
            </Surface>
            <Surface style={styles.infoBox}>
              <Title>Pain Frequency</Title>
              <Paragraph style={styles.textTitleCase}>
                <MaterialCommunityIcons name="calendar" size={16} /> {painData.getRecurrenceString()}
              </Paragraph>
            </Surface>
          </View>
          <Surface>
